...
class TOCGenerator:
    ...
    def create_table_of_contents(self, styles, use_actual_pages=False):
        ...
        if use_actual_pages and self.actual_page_numbers:
            ...
            return self._create_toc_with_pages(styles, story)
        else:
            print("  ↳ Creating TOC without page numbers (first pass)")
            return self._create_toc_without_pages(styles, story)
    
    def _create_toc_with_pages(self, styles, story):
        """Create TOC with actual page numbers from first pass"""
    ...
    def _create_toc_without_pages(self, styles, story):
        """Create TOC without page numbers for first pass"""
        for item in self.toc_items:
            level = item['level']
            text = item['text']
